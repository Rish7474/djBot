...
class ActionHandler {
  ...
  processAction(eventInfo, cmdQuery) {
    ...
    for (let actionType of actionTypes) {
      if (
        actionType.TYPE == "CMD" &&
        (actionType.INVOKE_LIST.includes(cmd) ||
          actionType.SHORTCUT_INVOKE.includes(cmd))
      ) {
  ...
  processNaturalLanguage(eventInfo, cmdQuery, botName) {
    ...
    if (nameIndex == -1) return ACTION_LIST.NULL.STATUS_HANDLE();

    const actionTypes = Object.values(ACTION_LIST);
    for (
      let i = nameIndex + 1;
      i < tokens.length && commandStatus == undefined;
      i++
    ) {
      for (const actionType of actionTypes)
        if (
          actionType.TYPE == "CMD" &&
          actionType.INVOKE_LIST.includes(tokens[i])
        ) {
          commandStatus = this.processAction(
            eventInfo,
            tokens.slice(i).join(" ")
          );
          break;
    ...
    if (commandStatus == undefined) return ACTION_LIST.NULL.STATUS_HANDLE();
    return commandStatus;
  }

  _getTokens(query) {
    if (query.length == 0) return [undefined, undefined];
